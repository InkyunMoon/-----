...
# plate_no
sum(raw_data['PLATE_NO'].value_counts().values == 1) # 데이터별로 유니크한 값을 가진다.
...
# PT_WDTH
# FUR_HZ_TIME
# FUR_SZ_TIME 이 네가지, 1.5IQR을 벗어나는 데이터 다수 존재. 4개 피쳐에서 동시에 2개 이상의 피쳐가 fence를 벗어나는 경우, 삭제하도록 하자.
...
detect_outliers(raw_data, 2, ['PT_THK','PT_WDTH','FUR_HZ_TIME','FUR_SZ_TIME']) # 두개 이상 피쳐에서 아웃라이어가 있는 인덱스 제거
